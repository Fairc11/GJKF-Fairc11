...
| 症状 | 排查路线 |
|------|---------|
| 响应慢 | 先 profile 找到瓶颈 → 是 IO 密集型还是 CPU 密集型？→ IO 密集型用并行/异步 → CPU 密集型考虑缓存或算法优化 |
| 启动慢 | 检查 import 耗时 → 考虑懒加载 → 异步初始化非必要组件 |
| 内存占用高 | 检查大对象引用是否及时释放 → 是否存在循环引用 → 用 `gc` 模块调试 |
| 工具"转圈不动" | 检查是否有长时间阻塞操作没有反馈 → 加进度回调或日志 → 设超时 |

### G. 环境问题排查

| 症状 | 排查路线 |
|------|---------|
| "我电脑能跑，他那不行" | 检查 Python 版本是否一致 → 依赖版本是否锁定 → 系统编码是否不同 |
| 打包后路径不对 | 用 `sys.frozen` + `sys._MEIPASS` 做环境判断，不要写死路径 |
| 编码乱码 | 所有文件打开指定 `encoding='utf-8'` → 控制台输出前检查 `sys.stdout.encoding` |
| 缺少组件 | 启动时自动检测 → 缺失则自动下载 → 下载失败给中文提示 |

### H. 外部服务排查

| 症状 | 排查路线 |
|------|---------|
| API 调用失败 | 检查网络连通性 → 检查认证是否过期 → 检查参数格式 → 检查是否触发限流 |
| 页面提取失败 | 检查页面结构是否更新 → 调整等待/超时 → 检查是否触发了反爬 |
| 下载文件 403 | 检查 Referer/User-Agent → 考虑加 CDN 代理转发 → 检查防盗链策略 |
| 认证频繁失效 | 检查 token 刷新机制 → 检查存储路径在打包后是否还可用 → 检查并发冲突 |
